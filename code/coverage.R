...
if(FALSE){
  all_agg_50 <- all_agg_90 <-matrix(NA, nrow = 4416, ncol = 55)
  all_bot_50 <- all_bot_90 <-matrix(NA, nrow = 4416, ncol = 1578)
  ...
  for(idtest in seq(4416)){
    ...
    all_agg_90[idtest, ] <-  (obs_agg_idtest >= QF_agg_idtest[which(taus == 0.05), ]  & obs_agg_idtest <= QF_agg_idtest[which(taus == 0.95), ]) 
    all_agg_50[idtest, ] <- (obs_agg_idtest >= QF_agg_idtest[which(taus == 0.25), ]  & obs_agg_idtest <= QF_agg_idtest[which(taus == 0.75), ]) 
    
    all_bot_90[idtest, ] <- (obs_bottom_idtest >= QF_bottom_idtest[which(taus == 0.05), ]  & obs_bottom_idtest <= QF_bottom_idtest[which(taus == 0.95), ]) 
    all_bot_50[idtest, ] <- (obs_bottom_idtest >= QF_bottom_idtest[which(taus == 0.25), ]  & obs_bottom_idtest <= QF_bottom_idtest[which(taus == 0.75), ]) 
    
  }
  apply(all_agg_90, 2, mean)
  apply(all_agg_50, 2, mean)
  
  plot(apply(all_bot_90, 2, mean))
  plot(apply(all_bot_50, 2, mean))
  ...
  #bot_colors <- c("black", "red", "green", "purple")
  #agg_colors <- c("black", "blue", "orange", "red", "grey", "green", "purple")
