...
predictkde <- function(task = c("learning", "testing", "insample_info"), selected_bandwiths = NULL, selected_lambdas = NULL){
  ...
  if(task == "learning"){
    ...
    #### REMOVE !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
    #bandwiths <- seq(10^-5, 0.01, by = 0.0005)
    
    #bandwiths <- seq(10^-7, 10^-3, by = 5 * 10^-5)
    #browser()
...
kde <- function(id_query, ids_data, bandwiths, lambda, task){
  ...
  for(i in seq_along(bandwiths)){
    ...
    if(mykernel == "normal"){
      ...
      #print(paste(length(ids_boundary), " / ", length(x), sep = ""))
      #print(vech)
    }
    ...
    if(task != "insample_info"){
      if(mykernel == "normal"){
        ...
        #x <- matrix(x, ncol = n)
        #vech <- matrix(vech, ncol = n)
        #normalized_weights <- matrix(normalized_weights, ncol = n)
        #vec_crps[i] <- crps(y = obs, family = "mixnorm", m = x, s = sqrt(vech), w = normalized_weights)
