...
for(k in seq(length(mylist) -1)){
  if(do.seperate){
    if(do.mse && k %in% c(1, 3)){
      savepdf(file.path(results.folder, paste("RESULTS_1bis_", k, sep = "")))
      #par(mfrow = c(1, 2))
      par(mfrow = c(2, 2))
    }else if(!do.mse){
      savepdf(file.path(results.folder, paste("RESULTS_1_", k, sep = "")))
      par(mfrow = c(2, 2))
    }
  }
  print(k)
