...
for(myrow in 1:2){
  ...
  for(step in 1:2){
    ...
    for(i_measure in seq_along(measures)){
      ...
      list_mat <- get_mat(measure, do.skill = TRUE)
      ...
      if(step == 1){
        ...
        matplot(x = c(x_vec, x_vec_bot), y = rbind(y_mat, y_mat_bot), ylim = my_range, cex = .5,
                type = 'o', pch = pch.agg[id_wanted_agg],  lty = 1, col = "black", #col = color.agg[id_wanted_agg],
                xlab = "Log(1 + Kwh)", ylab = paste(measure, " skill", sep = ""), main = paste(measure, " skill", sep = ""))
        if(measure == measures[1]){
          legend("bottomleft", agg_methods[id_wanted_agg], lty = 1, pch = pch.agg[id_wanted_agg])
        }
