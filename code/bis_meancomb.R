...
for(iseries in alliseries){
  ...
  my_hour_learn <- as.factor(calendar$periodOfDay[learn$id])
  my_hour_test <- as.factor(calendar$periodOfDay[test$id])
  ...
  my_hour_learn <- my_hour_learn[ikeep] 
  my_hour_test <- my_hour_test[ikeep] 
  
  for(h in seq(48)){
    id <- which(my_hour_learn == h)
    
    Xhat_learn_h <- Xhat_learn[id ,]
    y_learn_h <- y_learn[id]
    
    nlearn <- nrow(Xhat_learn_h)
    itrain <- seq(round(0.75*nlearn)) 
    ivalid <- setdiff(seq(nlearn), itrain)
    
    Xhat_train_h <- Xhat_learn_h[itrain, ]; y_train_h <- y_learn_h[itrain]
    Xhat_valid_h <- Xhat_learn_h[ivalid, ]; y_valid_h <- y_learn_h[ivalid]
